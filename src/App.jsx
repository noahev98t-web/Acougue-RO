...
// Envia o arquivo do comprovante pro Storage do Supabase e devolve o link público.
async function uploadComprovante(numero, file) {
  if (!file) return null;
  const ext = (file.name.split(".").pop() || "jpg").toLowerCase();
  const path = `numero-${numero}-${Date.now()}.${ext}`;
  ...
  return `${SUPABASE_URL}/storage/v1/object/public/comprovantes/${path}`;
}
...
export default function SorteioAcougue() {
  ...
  const [form, setForm] = useState({ nome: "", whats: "", comprovante: null });
  ...
  function pickNumber(n) {
    ...
    setForm({ nome: "", whats: "", comprovante: null });
  }
  ...
  async function confirmReservation() {
    ...
    try {
      ...
      if (form.comprovante) {
        ...
        comprovante_url = await uploadComprovante(selected, form.comprovante);
      }
      await patchNumber(selected, { status: "reservado", nome: form.nome, whats: form.whats, expires_at, comprovante_url });
      showToast(`Número ${selected} reservado por 15 minutos.`);
      setSelected(null);
      refresh();
    } catch (e) {
      showToast("Não foi possível reservar: " + e.message);
    }
  }
      ...
      {selectedNum && (
        <div style={S.overlay} onClick={() => setSelected(null)}>
          <div style={S.modal} className="fadein" onClick={(e) => e.stopPropagation()}>
            ...
            <label style={S.label}>Comprovante do Pix (opcional)</label>
            <input
              style={S.input}
              type="file"
              accept="image/*,.pdf"
              onChange={(e) => setForm({ ...form, comprovante: e.target.files[0] || null })}
            />
      ...
      <section style={S.panelCard}>
        ...
        {reservas.map((r) => (
          <div key={r.numero} style={S.rowItem}>
            <div>
              <strong>Número {r.numero}</strong> — {r.nome} · {r.whats}
              {r.comprovante_url && (
                <div>
                  <a href={r.comprovante_url} target="_blank" rel="noreferrer" style={S.linkBtn}>Ver comprovante</a>
                </div>
              )}
            </div>
